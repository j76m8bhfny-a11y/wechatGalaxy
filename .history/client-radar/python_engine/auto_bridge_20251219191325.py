...
def get_interactions(cursor, feed_id):
    ...
    try:
        # A. 动态探测表名：优先找 CommentV20，其次找 SnsComment
        table_name = "SnsComment" # 默认值
        ...
        columns_set = {col[1] for col in columns_info} # 转成集合方便查找
        
        # C. 映射列名 (不同版本的列名可能不同)
        # 目标: UserName, NickName, Content, CreateTime, Type, ReplyUserName
        
        # 1. 评论类型列 (Type 或 CommentType)
        type_col = "Type" if "Type" in columns_set else "CommentType"
        
        # 2. 回复人列 (ReplyUserName 或 RefUserName 或可能没有)
        reply_col = "ReplyUserName"
        if "ReplyUserName" not in columns_set:
            if "RefUserName" in columns_set: reply_col = "RefUserName"
            else: reply_col = "NULL" # 如果没有，就选 NULL 占位
            
        # 3. 内容列
        content_col = "Content"
        
        # D. 构建动态 SQL
        # 注意：ReplyUserName 可能被映射为 NULL
        sql = f"""
            SELECT UserName, NickName, {content_col}, CreateTime, {type_col}, {reply_col}
            FROM {table_name} 
            WHERE FeedId=? 
        ...
        # 调试用：如果出错，打印错误但不崩溃 (实际生产环境可以去掉 print)
        # print(f"Interactions Error: {e}") 
        pass
        
    return likes, comments

# 4. 主读取逻辑 (也要适配 FeedsV20)
