...
const autoScan = async () => {
  ...
  contactStore.errorMsg = ''; // 清除旧报错
  
  try {
    console.log("正在启动自动扫描...");
    // 调用我们在 Rust 中写的 auto_decrypt_wechat 命令
    const resStr = await invoke<string>('auto_decrypt_wechat');
    console.log("扫描完成，原始返回:", resStr);
    ...
    if (res.status === 'success') {
      // 1. 自动填入路径给用户看
      dbPathInput.value = res.micro_db_path;
      
      // 2. 自动触发读取联系人 (使用 Store 的逻辑)
      console.log("正在导入通讯录数据库...");
      await contactStore.importFromDb(res.micro_db_path);
      
      // (未来扩展: 这里也可以保存 res.sns_db_path 用于读取朋友圈)
      console.log("✅ 自动化流程全部成功！");
    } else {
      // Python 脚本返回了 error 状态
      contactStore.errorMsg = res.message || '扫描返回未知错误';
    }
    
  } catch (e) {
    console.error("扫描过程发生异常:", e);
    contactStore.errorMsg = "扫描失败: " + String(e);
  } finally {
